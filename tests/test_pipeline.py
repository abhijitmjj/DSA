...
def mock_ml_client_from_config(mocker):
    ...
    mock_client_instance.compute.get.return_value = mock.MagicMock() # for _get_or_create_compute_target
    mock_client_instance.environments.get.return_value = mock.MagicMock(spec=Environment) # for _get_default_environment

    mock_from_config = mocker.patch('utils.pipeline.MLClient.from_config', return_value=mock_client_instance)
...
@pytest.fixture
def pipeline_instance(mocker, mock_default_azure_credential, mock_ml_client_from_config, minimal_pipeline_params):
    ...
    mocker.patch('utils.pipeline.Pipeline._get_or_create_compute_target', return_value="test-cluster")
    # _get_default_environment returns an Environment object
    mock_env = mock.MagicMock(spec=Environment)
    mocker.patch('utils.pipeline.Pipeline._get_default_environment', return_value=mock_env)
    
    return Pipeline(**minimal_pipeline_params)

class TestPipelineInitialization:
    def test_init_success_with_specific_config_file(
        self, mocker, mock_default_azure_credential, mock_ml_client_from_config, minimal_pipeline_params
    ):
    ...
    def test_init_success_with_default_config_file(
        ...
        mock_from_config.assert_called_once_with(credential=mock_default_azure_credential.return_value)
        
        assert pipeline.ml_client is not None
        assert pipeline.ml_client.workspace_name == "mock_workspace"

    def test_init_success_with_env_vars(
        self, mocker, mock_default_azure_credential, mock_ml_client_constructor, minimal_pipeline_params
    ):
        """Tests successful initialization using environment variables when config files fail."""
        mocker.patch('pathlib.Path.resolve', return_value=pathlib.Path(MOCK_CONFIG_PATH))
        mocker.patch('pathlib.Path.is_file', return_value=False) # Specific config file not found
        
        # Make MLClient.from_config raise an exception to trigger env var fallback
        mocker.patch('utils.pipeline.MLClient.from_config', side_effect=Exception("Config load failed"))
        ...
        mock_ml_client_instance_env.environments.get.return_value = mock.MagicMock(spec=Environment)
    ...
    def test_init_compute_and_env_creation_mocked(
        ...
        mock_get_compute = mocker.patch('utils.pipeline.Pipeline._get_or_create_compute_target', return_value="test-cluster")
...
class TestPipelineStepManagement:
    def test_add_simple_step(self, pipeline_instance, mock_command_func):
        ...
        assert isinstance(kwargs["environment"], mock.MagicMock) # From _get_default_environment mock

        assert step_name in pipeline_instance._step_definitions
        component_factory, defined_inputs = pipeline_instance._step_definitions[step_name]
    ...
    def test_add_step_with_inputs_outputs(self, pipeline_instance, mock_command_func):
        ...
        outputs = {"processed_data": Output(type=AssetTypes.URI_FOLDER, path="/test/processed/")}
    ...
    def test_get_output_success(self, pipeline_instance, mock_command_func):
        ...
        outputs = {output_name: Output(type=AssetTypes.URI_FOLDER, path="/test/produced/")}

        pipeline_instance.add_step(name=step_name, script_path=script_path, outputs=outputs)
        ...
        assert input_for_next_step.path == f"${{{{parent.jobs.{step_name}.outputs.{output_name}}}}}"
...
class TestPipelineSubmission:
    ...
    def test_submit_single_step(self, pipeline_instance, mock_command_func):
        ...
        assert pipeline_job_arg.jobs[step_name] is mock_node # Check the node is in the job
        assert submitted_job is mock_submitted_job

    def test_submit_multi_step_with_dependency(self, pipeline_instance, mock_command_func):
        ...
        mock_command_func.side_effect = [producer_component_factory, consumer_component_factory]
        ...
        consumer_inputs = {
            consumer_input_name: pipeline_instance.get_output(producer_step_name, producer_output_name)
        }
        pipeline_instance.add_step(
            name=consumer_step_name, 
